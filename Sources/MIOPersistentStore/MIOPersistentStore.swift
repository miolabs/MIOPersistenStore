...
@objc public protocol MWSPersistentStoreDelegate : NSObjectProtocol {
    func webStore(_ store:MWSPersistentStore, requestForEntityName entityName:String, url:URL, bodyData:Data?, httpMethod:String) -> MWSRequest
    
    func webStore(store:MWSPersistentStore, fetchRequest:NSFetchRequest<NSFetchRequestResult>, serverID:String?) -> MWSRequest?
    ...
    func webStore(store: MWSPersistentStore, serverIDForItem item:Any, entityName: String) -> String
    func webStore(store: MWSPersistentStore, serverVersionNumberForItem item:Any, entityName: String) -> UInt64
    func webStore(store: MWSPersistentStore, serverEntityNameForItem item:Any, entityName: String) -> String
    
    func webStore(store: MWSPersistentStore, serverAttributeName: String, forEntity entity: NSEntityDescription) -> String
    func webStore(store: MWSPersistentStore, serverRelationshipName: String, forEntity entity: NSEntityDescription) -> String
    
    func webStore(store: MWSPersistentStore, valueForAttribute attribute: NSAttributeDescription, serverValue:Any?) -> Any?
...
public enum MWSPersistentStoreError :Error {
    case NoStoreURL
    case InvalidRequest
    case ServerIDIsNull
}

public class MWSPersistentStore: NSIncrementalStore {
    
    public override var type: String {return "MWSPersistentStore"}
    
    @objc public weak var delegate:MWSPersistentStoreDelegate?
    
    var storeURL:URL?
    var entitiesInfo = [String:Any]()
    var itemsByReferenceID = [String:Any]()
    var objectIDWithReferenceID = [String:Any]()
    var cache : MWSPersistentStoreCache!
    
    var contextByRequest = [NSFetchRequest<NSFetchRequestResult>:NSManagedObjectContext]()
    
    @objc public func fetchRequest(withEntityName entityName:String, context:NSManagedObjectContext) -> NSFetchRequest<NSFetchRequestResult>{
    ...
    @objc public func performFetchInFetchedResultsController(fetchResultsController:NSFetchedResultsController<NSFetchRequestResult>) throws {
    ...
    public override func loadMetadata() throws {
        ...
        let uuid = UUID.init()
        let metadata = [NSStoreUUIDKey: uuid.uuidString, NSStoreTypeKey: "MWSPersistentStore"]
        self.metadata = metadata
    }
    
    
    var persistentStoreCache:MWSPersistentStoreCache?
    
    @objc public func enableCache(model:NSManagedObjectModel, name:String){
        persistentStoreCache = MWSPersistentStoreCache(name:name, model: model, store: self)
    }
...
//            relationshipValuesByReferenceID.removeObject(forKey: referenceID)
    ...
    @objc public func fetchObjects(fetchRequest:NSFetchRequest<NSFetchRequestResult>, with context:NSManagedObjectContext, completion:FetchObjectsCompletion?) -> [Any] {
